...
$settings = array(
	...
	array(
		...
		'content' => '<button type="button" class="button button-primary wrap-spin gateway-payment-list" data-gateway="duitku">Refresh Payment List<i class="dashicons dashicons-update-alt"></i></button><div style="margin-top:5px">' . $text . '</div>',
	),
	array(
		'id'      => $pref . 'sandbox',
		'type'    => 'switcher',
		'title'   => __( 'Enable Testing Sandbox', 'wds-notrans' ),
		'desc'    => __( 'Enable sandbox only to test payments with duitku sandbox', 'wds-notrans' ),
	...
	array(
		...
		'desc'    => __( 'Jika diaktifkan, link pembayaran pada notifikasi email dan whatsapp diarahkan ke website Duitku.', 'weddingsaas' ),
...
if ( wds_check_array( $duitku, true ) ) {
	foreach ( $duitku as $key => $title ) {
		$channels[] = array(
			'type'  => 'subheading',
			'title' => $title . ' (' . strtoupper( str_replace( 'duitku_', '', $key ) ) . ')',
		);
		$channels[] = array(
			'id'      => $key . '_icon_enable',
			'type'    => 'switcher',
