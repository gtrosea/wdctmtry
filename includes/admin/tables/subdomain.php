 ...
 * Subdomain Class.
 *
 * A custom WordPress List Table to display and manage subdomains in the admin dashboard.
 */
class Subdomain extends \WP_List_Table {
	...
	/**
	 * Get available views (filters) for subdomains.
	 *
	...
	protected function get_views() {
		...
		$link = menu_page_url( 'wds-replica-subdomain', false );
		if ( ! $current_status ) {
	...
	/**
	 * Checkbox column for selecting subdomains.
	 *
	 * @param object $item The subdomain item.
	...
	/**
	 * Name column for the subdomain.
	 *
	 * @param object $item The subdomain item.
	 * @return string The HTML for the name.
	 */
	protected function column_name( $item ) {
		$url = $item->subdomain . '.' . wds_host();

		return '<a href="https://' . $url . '" target="_blank">' . $url . '</a>';
	}

	/**
	 * User column for the subdomain.
	 *
	 * @param object $item The subdomain item.
	...
	/**
	 * Status column for the subdomain.
	 *
	 * @param object $item The subdomain item.
	...
	protected function column_status( $item ) {
		...
		return sprintf(
			'<span style="color:' . $color . ';">%1$s</span>',
			ucfirst( $item->status )
		);
	}
	...
	/**
	 * Process the bulk actions for the subdomains.
	 */
	protected function process_bulk_action() {
		...
		if ( check_admin_referer( 'bulk-' . $this->_args['plural'] ) ) {
			$subdomains = isset( $_GET['subdomain'] ) ? (array) $_GET['subdomain'] : array();

			if ( 'deleted' === $this->current_action() ) {
				foreach ( $subdomains as $key => $id ) {
					wdr_delete( $id );
				}
			}
	...
	/**
	 * Prepare the items (subdomains) for the table.
	 */
	public function prepare_items() {
		...
		if ( $search ) {
			$sql .= sprintf(
				" subdomain LIKE %s OR user_id IN (SELECT ID FROM {$wpdb->users} WHERE display_name LIKE %s)",
				"'%$search%'",
				"'%$search%'"
			);
		}
		...
		if ( $sql ) {
			$sql .= " AND subdomain != 'nothing'";
		} else {
			$sql .= " subdomain != 'nothing'";
		}
