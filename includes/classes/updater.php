 ...
 * WeddingSaas Updater.
 *
 * This class handles the updater of the plugin.
 *
 * @since 1.0.0
 * @package WeddingSaas
 * @subpackage Classes
...
class WDS_Updater {
	...
	public function show_update_notification( $file, $plugin ) {
		...
		printf(
			...
			esc_html__( 'Versi baru %1$s telah tersedia.', 'weddingsaas' ),
			esc_html( $plugin['Name'] )
		);

		if ( ! current_user_can( 'update_plugins' ) ) {
			echo ' ';
			esc_html_e( 'Hubungi administrator jaringan Anda untuk menginstal pembaruan.', 'weddingsaas' );
		} elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
			echo ' ';
			printf(
				/* translators: 1. opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
				__( '%1$sLihat detail versi %2$s%3$s.', 'weddingsaas' ),
			...
			echo ' ';
			printf(
				__( '%1$sLihat detail versi %2$s%3$s atau %4$sperbarui sekarang%5$s.', 'weddingsaas' ),
				'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
				esc_html( $update_cache->response[ $this->name ]->new_version ),
				'</a>',
			...
			printf(
				' %1$s%2$s%3$s',
				'<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
				esc_html__( 'Perbarui sekarang.', 'weddingsaas' ),
				'</a>'
			);
		}
	...
	public function show_changelog() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'Anda tidak memiliki izin untuk menginstal pembaruan plugin', 'weddingsaas' ), 'Error', array( 'response' => 403 ) );
		}
