...
if ( ! class_exists( 'CSF_Setup' ) ) {
  class CSF_Setup {
    ...
    public static function include_plugin_file( $file, $load = true ) {
      ...
      } elseif ( file_exists( WDS_INCLUDES . 'extends/' . $file ) ) {
        $path = WDS_INCLUDES . 'extends/' . $file;
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field_type ) ) {
        ...
        if ( ! empty( $field['title'] ) ) {
            ...
            if ( ! empty( $field['tutorial'] ) ) {
              echo '<a href="'.$field['tutorial'].'" target="_blank" class="link--primary"><i class="fas fa-question-circle"></i></a>';
            }
            echo '</h4>';
